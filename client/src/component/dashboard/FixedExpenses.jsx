...
const FixedExpenses = ({ fixedCostTransactions }) => {
    return (
        <div className="p-6 bg-white rounded-2xl shadow-lg border h-full flex flex-col">
            ...
            <div className="flex-grow space-y-3 overflow-y-auto max-h-32 pr-1 custom-scrollbar">
                {fixedCostTransactions?.data?.fixedCostItems.length > 0 
                    ?   fixedCostTransactions?.data?.fixedCostItems?.map((item, idx) => (
                    ...
                    :   <div className="bg-gray-50 border border-gray-100 rounded-lg p-4 text-center text-gray-500">
                            Không có khoản chi cố định nào trong tháng này.
                        </div>}
            </div>
            <div className="mt-3 pt-3 border-t border-gray-100">
                <div className="flex justify-between items-center mb-2">
                    <span className="font-bold text-gray-700">Tổng cộng</span>
                    <span className="font-bold text-blue-600 text-lg">{formatCurrency(fixedCostTransactions?.data?.totalAll)}</span>
                </div>
