...
const CalendarView = ({ transactions, onDayClick }) => {
    ...
    const transactionsByDate = useMemo(() => transactions.reduce((acc, tx) => {
        ...
        <div className="bg-white p-6 rounded-2xl shadow-md">
            ...
            <div className="grid grid-cols-7 gap-2 mt-2">
                {days.map((d, i) => {
                    ...
                    return (
                        <div key={i} onClick={() => onDayClick(d)} className={`p-2 rounded-lg h-24 flex flex-col ${isCurrentMonth ? 'bg-white hover:bg-blue-50 cursor-pointer' : 'bg-gray-50/70'} border transition-colors`}>
                            <span className={`font-bold self-start ${isCurrentMonth ? 'text-gray-700' : 'text-gray-400'}`}>{d.getDate()}</span>
                            {dailyTx.length > 0 && (
                                <div className="mt-1 space-y-1 overflow-y-auto text-xs scrollbar-hide">
                                    {dailyTx.map(tx => (
                                        <div key={tx.id} className={`p-1 truncate rounded ${tx.type === 'income' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{formatCurrency(tx.amount)}</div>
                                    ))}
                                </div>
                            )}
                        </div>
