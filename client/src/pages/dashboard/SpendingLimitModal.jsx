    ...
    return (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm flex flex-col animate-scale-up">
                <div className="flex justify-between items-center p-5 border-b bg-gradient-to-r from-emerald-50 to-teal-100">
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    ...
                    <div className="mb-4">
                        ...
                        <input type="number" value={limit} onChange={e => setLimit(e.target.value)} placeholder="Nhập số tiền" className="w-full p-2 text-lg font-bold border-2 border-emerald-200 rounded-lg focus:ring-emerald-500 focus:border-emerald-500" required />
                    </div>
                    <div className="mb-6">
                    ...
                    <button type="submit" className="w-full py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold rounded-lg hover:shadow-lg hover:opacity-90 transition-all flex items-center justify-center">{isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Lưu hạn mức'}</button>
                </form>
            </div>
        </div>
    );
